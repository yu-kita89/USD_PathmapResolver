...
struct _ArPathmapResolverFallbackContext {
    _ArPathmapResolverFallbackContext() {
        ...
        if (!envPath.empty()) {
            // PXR_AR_DEFAULT_SEARCH_PATHの値を区切って、コンテキストを作成;
            context = ArPathmapResolverContext(_ParseSearchPaths(envPath));
        }
    }
...
{
    // 新しい検索パスからコンテキストを作成
    ArPathmapResolverContext newFallback = ArPathmapResolverContext(searchPath);
    ...
    const std::string& path) const
{
    ...
    // 見つからない場合は空のパスを返す;
    ArResolvedPath resolvedPath = _ResolveAnchored(std::string(), path);
